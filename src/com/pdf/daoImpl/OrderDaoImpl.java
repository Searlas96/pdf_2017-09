...
//			}
	...
	public int AddFile(File file) {
		String sql = "insert into file (order_id,order_name,file_path,client_name,end_time,order_time,modify_time,edit_time,locked"
				+ ",delivery_time,order_state,edition,`schedule`,user_id,`explain`,remark,style,printed,quantity) "
				+ "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		...
		Object[] params = {file.getOrder_id(),file.getOrder_name(),file.getFile_path(),file.getClient_name(),
				...
				file.getStyle(),file.getPrinted(),file.getQuantity()
		}; 
		try {
			System.out.println(sql);
	...
	public int getNewId() {
		// 返回一个可以生成新文件的id
		int id = 0 ;
		String sql = "select IFNULL( Max(file_id) , 0 ) as file_id from file";
		try {
			Long id3 = runerQuery.query(sql, new ScalarHandler<Long>());
			id = id3.intValue();
			System.out.println("newid =============="+(id+1));
			return id+1;
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public int SubmitFile(File file) {
		...
		String sql2 = "insert into file (order_id,order_name,file_path,client_name,end_time,order_time,modify_time,edit_time,locked"
				+ ",delivery_time,order_state,edition,`schedule`,user_id,`explain`,remark,quantity,style,printed) "
				+ "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		//寻找新版本的id
		...
		Object[] params = {file.getOrder_id(),file.getOrder_name(),file.getFile_path(),file.getClient_name(),
				...
				file.getDelivery_time(),file.getOrder_state(),file.getEdition(),
				file.getSchedule(),file.getUser_id(),file.getExplain(),file.getRemark(),
				file.getQuantity(),file.getStyle(),file.getPrinted()
		};
		int result = 0;
	...
	public int UplaodFile(File file) { //先将旧版本edition设为0 再插入新版本file
		...
		String sql2 = "insert into file (order_id,order_name,file_path,client_name,end_time,order_time,modify_time,edit_time,locked"
				+ ",delivery_time,order_state,edition,`schedule`,user_id,`explain`,remark,quantity,printed,style) "
				+ "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		Object[] params = {file.getOrder_id(),file.getOrder_name(),file.getFile_path(),file.getClient_name(),file.getEnd_time(),file.getOrder_time()
				...
				file.getRemark(),file.getQuantity(),file.getPrinted(),file.getStyle()};
		int flag = 0;
		try {
	...
	@Override
	public int readOrder(String order_id, String user_id) {
		String sql = "insert into user_read (order_id,user_id) values (?,?)";
		Object[] params = {order_id,user_id};
		try {
			return runerQuery.update(sql,params);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	@Override
	public int editOrder(File file) {
		String sql = "update file set file.style = ?,file.printed = ?,file.quantity = ?"
				+ ",file.client_name = ?,file.end_time = ?,file.order_time = ?"
				+ ",file.delivery_time = ?,file.explain =?,file.remark = ?,file.user_id=? "
				+ "where file.order_id = ?";
		Object[] params = {file.getStyle(),file.getPrinted(),file.getQuantity(),file.getClient_name(),
				file.getEnd_time(),file.getOrder_time(),file.getDelivery_time(),file.getExplain(),
				file.getRemark(),file.getUser_id(),file.getOrder_id()};
		try {
			return runerQuery.update(sql,params);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	@Override
	public List<File> getWorkOrderList(String user_id, String role_id) {
		String sql1 = "select * from file where file.schedule = 0 and file.edition = 1"
				+ " and file.order_state = 1 and not exists "
				+ "(select ur.order_id from user_read as ur where ur.user_id = "+user_id
				+ " and file.order_id = ur.order_id )";
		String sql2 = "select * from file where file.schedule = 1 and file.edition = 1"
				+ " and file.order_state = 1 and not exists " 
				+ "(select ur.order_id from user_read as ur where ur.user_id = "+user_id
				+ " and file.order_id = ur.order_id )";
		try {
			if(role_id.equals("2")) {
				//编辑员
					...
					return runerQuery.query(sql1, new BeanListHandler<>(File.class));
			}else if(role_id.equals("3") || role_id.equals("4")) {
				//审核员和普通用户
				System.out.println(sql2);
					return runerQuery.query(sql2, new BeanListHandler<>(File.class));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	@Override
	public File searchOrderByName(String order_name) {
		String sql = "select * from file where file.order_name = ? and file.edition = 1 and file.order_state = 1";
		 try {
			return runerQuery.query(sql,new BeanHandler<>(File.class), order_name);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
