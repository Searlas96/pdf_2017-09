	...
	$.ajax({
		...
		success: function(json){
			...
			for(var i=0;i<json.result.length;i++){
				...
				var edittemp = "修改订单";
				var editurl = "Admin/Order/order-after.html?order_id="+json.result[i].order_id+"&user_id="+user_id+"&style="+json.result[i].style+"&printed="+json.result[i].printed+"&quantity="+json.result[i].quantity+"&client_name="+json.result[i].client_name+"&end_time="+json.result[i].end_time+"&order_time="+json.result[i].order_time+"&delivery_time="+json.result[i].delivery_time+"&explain="+json.result[i].explain+"&remark="+json.result[i].remark+"";
				var showtemp= "订单详情";
				...
				var aftertemp = "撤回进度";
				var afterurl = "Admin/Order/order-edit.html?order_id="+json.result[i].order_id+"&&user_id="+user_id+"&&edit_time="+json.result[i].edit_time+"&&locked="+json.result[i].locked+"&schedule="+json.result[i].schedule+"";//&&schedule="+json.result[i].schedule+"
				...
				if(json.result[i].schedule == 0){
					if(role_id== 1){
						...
						operation = '<td class="td-manage" ><span id="'+oper+'"> '+test+'<a class="'+rewider+' ml-5"   title="修改订单" href="javascript:;" onclick="order_edit('+'\''+edittemp+'\''+','+'\''+editurl+'\''+',800,510)" style="text-decoration:none;float:left"><i class="Hui-iconfont">&#xe6df;</i></a></span></td>';
						...
						state = "已录入";
						spanColor = "label label-danger radius";
						//operation = '<td class="td-manage" ><span id="'+oper+'"> '+test+'<a class="'+rewider+' ml-5"   title="撤回进度" href="javascript:;" onclick="order_edit('+'\''+edittemp+'\''+','+'\''+editurl+'\''+',800,510)" style="text-decoration:none;float:left"><i class="Hui-iconfont">&#xe6df;</i></a></span></td>';
					}else{
						state ="已录入";
						spanColor = "label label-danger radius";
						rewider="dip";
						operation = '<td class="td-manage" ><span id="'+oper+'"> '+test+'<a class="'+rewider+' ml-5"   title="修改订单" href="javascript:;" onclick="order_edit('+'\''+edittemp+'\''+','+'\''+editurl+'\''+',800,510)" style="text-decoration:none;float:left"><i class="Hui-iconfont">&#xe6df;</i></a> <a class=" ml-5"   title="撤回进度" href="javascript:;" onclick="order_after('+'\''+aftertemp+'\''+','+'\''+afterurl+'\''+',400,210)" style="text-decoration:none;"><i class="Hui-iconfont">&#xe6de;</i></a><a title="删除" href="javascript:;"  onclick="order_del(this,'+json.result[i].order_id+')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></span></td>';
