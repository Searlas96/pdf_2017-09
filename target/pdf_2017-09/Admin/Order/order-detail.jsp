...
<!--       <a href = "javascript:void(0)" onclick = "document.getElementById('light').style.display='block';document.getElementById('fade').style.display='block'" style="color:white">上传</a>  --> 
    <a href="javascript:;" onclick="order_upload('上传文件','../../Admin/Order/order-upload.jsp?file_path=<%=file_path %>&file_id=<%=file_id %>','510','310')" id="upload_order" style="color:white">上传</a>
  </span>
    <span style="display: block;" class="updateBtn" id="ResumeInfo_Button">
      <a href="../../order.jhtm?method=DownloadForModify&file_id=<%=file_id %>&order_id=<%=order_id %>" style='color:white;' onclick="tomd()">下载与修改</a>    
   </span>	
  ...
  <table class="table" id="ta1">
    <tbody>
      ...
      <tr>
       ...
       <tr>
        <th class="text-r">印花：</th>
        <td id="printed"></td>
      </tr> <tr>
        <th class="text-r">款式：</th>
        <td id="style"></td>
      </tr>
      <tr>
...
function order_upload(title,url,w,h){
